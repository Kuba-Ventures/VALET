...
export function createOrb(canvas: HTMLCanvasElement): Orb {
  ...
  const mat = new THREE.PointsMaterial({
    color: 0x6e5cff, size: 0.4, transparent: true, opacity: 0.6,
  ...
  const lineMat = new THREE.LineBasicMaterial({
    color: 0x6e5cff, transparent: true, opacity: 0.0,
  ...
  function animate() {
    ...
    // Particles + lines tint together — purplish-blue with subtle per-state shifts.
    if (state === "thinking") {
      mat.color.lerp(new THREE.Color(0x9080ff), 0.015);
      lineMat.color.lerp(new THREE.Color(0x9080ff), 0.015);
    } else if (state === "speaking") {
      mat.color.lerp(new THREE.Color(0x7c68f0), 0.015);
      lineMat.color.lerp(new THREE.Color(0x7c68f0), 0.015);
    } else {
      mat.color.lerp(new THREE.Color(0x6e5cff), 0.015);
      lineMat.color.lerp(new THREE.Color(0x6e5cff), 0.015);
    }
