...
async def lifespan(application: FastAPI):
    ...
    # Global push-to-talk (⌃⌥) now lives in the Tauri MAIN process (src-tauri,
    # `spawn_global_chord` via rdev), NOT here. macOS Input Monitoring is granted
    # per executable; the bundled backend helper's hash changes every build, so its
    # grant churned and the tap silently received no key events. The main app binary
    # is one stable, promptable identity ("VALET"), so the listener moved there and
    # drives the same frontend PTT path via a `window.__valetChord` hook. The legacy
    # `global_ptt.py` CGEventTap is intentionally no longer started.
...
_SUMMARIZE_SCREEN_RE = re.compile(
    ...
    r')\s*[.?!]*$',
    re.IGNORECASE)
# Batch-read today's Gmail inbox (issue #285): open each of today's messages, read
# each, and speak ONE combined digest. Anchored on BATCH verbs (go through / catch
# me up on / read me / summarize / recap / digest) + an inbox-scope noun (inbox /
# emails / mail / messages), so it claims "summarize my emails" (plural, whole
# inbox) BEFORE _SUMMARIZE_SCREEN_RE would read it as the one focused email — while
# "summarize THIS email" (deictic, no my/the/all) still falls to the single-email
